...
# -*- coding: utf-8 -*-
"""
WorkBuddy 每日签到自动领取积分脚本 (macOS 版 v3.7)
================================================
...
v3.7 修复（2026-09-16）：
  - 【根因修复】WorkBuddy 5.5.6 头像菜单重构，"Buddy加油站" 从 ratio 0.339
    下移至 0.385。旧坐标点中的是"积分余额"，打开 设置→套餐与积分 面板，
    签到卡片永不出现 → "导航后仍未找到签到入口" 连续失败（09-12~09-16）。
  - 实测确认：新菜单点头像 → no-move 点击 (0.088, 0.385) → 底部弹出白底
    签到卡片（"高校新生攻略/Buddy加油站·N期"），黑色实心「立即领取」按钮
    位于窗口 ratio y≈0.86-0.90，仍在 CARD_SEARCH 范围内，
    find_dark_button (threshold 55) 可直接命中，检测逻辑无需改动。
...
# "Buddy加油站" 菜单项（头像下拉菜单中第 3 项）
#   v3.7 (2026-09-16): 5.5.6 菜单重构为「体验版/积分余额/Buddy加油站/主邀约/
#   成长计划/设置/...」，Buddy加油站 下移至 ratio 0.385（实测 y≈614）。
#   旧值 0.339 (y≈579) 会点到"积分余额"，打开 设置→套餐与积分 面板
#   （09-12 ~ 09-16 连续失败的根因）。
#   ⚠ 必须用 cg_click_nomove（不预先移动光标）：cg_click 会先把光标移到
#   目标，光标离开头像锚点 → Electron 下拉菜单立即关闭，点击落到下方
#   侧边栏项（曾因此误触"定时任务"页）。
MENU_BUDDY_RATIO = (0.088, 0.385)
...
def run_checkin(debug=False, dry_run=False):
    ...
    """
    logging.info("=" * 50)
    logging.info("WorkBuddy 每日签到开始 (v3.7 — 5.5.6 菜单坐标校准)")
